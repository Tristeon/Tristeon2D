...
namespace Tristeon
{
	...
	{
		body = getOwner()->getBehaviour<PhysicsBody>();
	}
	...
	{
		getOwner()->position = Math::orbit(GameView::screenToWorld(Mouse::position()), Vector2{ 500, 500 }, r++);
		...
		bool grounded = PhysicsWorld::raycast(getOwner()->position, Vector2::down(), groundedDistance);
		...
		Camera::main()->position = (Vector2Int)getOwner()->position;
	}
